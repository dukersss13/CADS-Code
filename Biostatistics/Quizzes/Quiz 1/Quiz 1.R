...
boxplot(avg.mpg ~ manufacturer)
# From looking at the avg mpg by manufacturer in this dataset, Volkswagen seems
# to have a few outliers if we base it off the boxplot. Further analysis can be done
# using other statistical tests to examine this hypothesis.
...
# This model that resulted from the 2-way stepAIC function produces results very
# similar to our previous model in 1d. This is plausible since the AIC difference
# between the two models si not significant. We can once again analyze the coefficients
# of each predictor or their p-values to measure their level of significance.

# 1j ####

# For this particular case, I would probably prefer the second model (from 1g) since
# it does get the job done with more simplicity. 